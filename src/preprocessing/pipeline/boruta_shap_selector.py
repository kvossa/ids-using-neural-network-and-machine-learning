...
BINARY_FEATURES = [
    "spkts", "dpkts", "sbytes", "dbytes", "sinpkt", "dinpkt",
    "sload", "dload", "sjit", "djit", "stcpb", "dtcpb",
    "tcprtt", "smean", "dmean",
]

MULTICLASS_FEATURES = [
    "rate", "sttl", "dttl", "sloss", "dloss", "swin", "dwin",
    "synack", "ackdat", "trans_depth", "response_body_len",
...
FIXED_FEATURES = sorted(set(BINARY_FEATURES + MULTICLASS_FEATURES + ENGINEERED_FEATURES + HIGH_IMPORTANCE_FEATURES))


class FixedFeatureSelector(BaseEstimator, TransformerMixin):
    def __init__(self, features=None, dataset="UNSW"):
        self.features = features or FIXED_FEATURES
        self.dataset = dataset.upper()
        self.selected_features_ = None
        self.missing_features_ = []
        self.extra_features_ = []

    def fit(self, X, y=None):
        if hasattr(X, "columns"):
            available = set(X.columns.tolist())
            target = set(self.features)

            self.missing_features_ = sorted(target - available)
            self.extra_features_ = sorted(available - target)

            if self.missing_features_:
                print(f"[FixedFeatureSelector] Missing from raw data: {self.missing_features_}")
                print(f"[FixedFeatureSelector] These must be created by FeatureExtraction")

            self.selected_features_ = [f for f in self.features if f in available]

        else:
            self.selected_features_ = self.features

        print(f"[FixedFeatureSelector] Selected {len(self.selected_features_)} features")
        return self
...
# CIC baseline manual list (safe starter set; can be overridden from CLI)
CIC_MANUAL_FEATURES = [
    "Flow Duration",
    "Tot Fwd Pkts",
    "Tot Bwd Pkts",
...
def default_manual_features_for_dataset(dataset: str):
    ds = dataset.upper()
    if ds == "CIC":
        return CIC_MANUAL_FEATURES
    return FIXED_FEATURES


class ManualFeatureSelector(BaseEstimator, TransformerMixin):
    def __init__(self, dataset="CIC", features=None, strict=False):
        self.dataset = dataset.upper()
    ...
    def fit(self, X, y=None):
        base = self.features or default_manual_features_for_dataset(self.dataset)
        available = set(X.columns.tolist())
        target = list(dict.fromkeys(base))
        self.missing_features_ = sorted([f for f in target if f not in available])
        self.selected_features_ = [f for f in target if f in available]
        if self.missing_features_:
            print(f"[ManualFeatureSelector] Missing features ({len(self.missing_features_)}): {self.missing_features_}")
        if self.strict and self.missing_features_:
            raise ValueError("Strict manual selection failed: missing requested features.")
        print(f"[ManualFeatureSelector] Selected {len(self.selected_features_)} features")
        return self
...
class ManualHybridFeatureSelector(BaseEstimator, TransformerMixin):
    def __init__(self, dataset="CIC", features=None, k_features=30, random_state=42):
        self.dataset = dataset.upper()
        self.features = features
        self.k_features = k_features
        self.random_state = random_state
        self.manual_ = None
        self.ml_selector_ = None
        self.selected_features_ = None

    def fit(self, X, y):
        self.manual_ = ManualFeatureSelector(dataset=self.dataset, features=self.features, strict=False)
        X_manual = self.manual_.fit_transform(X, y)
        if X_manual.shape[1] == 0:
            raise ValueError("ManualHybridFeatureSelector found zero usable features.")
        k = min(self.k_features, X_manual.shape[1])
        self.ml_selector_ = FeatureSelector(
            k_features=k,
            random_state=self.random_state,
            use_anova=False,
            use_mi=False,
        ...
        self.ml_selector_.fit(X_manual, y)
        self.selected_features_ = self.ml_selector_.get_selected_features()
        print(f"[ManualHybridFeatureSelector] Final selected features: {len(self.selected_features_)}")
        return self

...
class MultiClassLabelEncoder(BaseEstimator, TransformerMixin):
	...
	def fit(self, X, y=None):
		...
		for index, value in self.class_to_index_.items():
			print(f"	{index} -->	{value}")
	...
	def transform(self, X):
		...
		labels_mapped = X_str.map(self.class_to_index_)

		if pd.Series(labels_mapped).isna().any():
			unseen = X_str[labels_mapped.isna()].unique()
